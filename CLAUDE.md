...
CLI / MCP / HTTP API / FUSE / NFS / Python   (access surfaces, one crate each)
        ...
        ▼
afs-sdk::Workspace          ergonomic façade; `open_*` constructors wire backends
        ▼
afs-core::Fs<M, C>          the working-tree engine — POSIX ops, chunking, commit,
...
attribution only), `native` (afs's own chunked commit DAG — the default when a
...
| `afs-sdk` | `Workspace` — the ergonomic façade over `afs-core::Fs`. The API every other surface calls. |
...
| `afs-mcp` | MCP server — agents call filesystem tools over stdio, auto-attributed. |
| `afs-git` | Real-`git` interop: export/import genuine git objects; ships the `git-remote-afs` helper binary (`git clone afs://…`). |
