//! `git-remote-afs` — a git remote helper that lets the real `git` clone, fetch,
//! and push an afs workspace over `afs://` URLs (`docs/DESIGN.md` §4c interop
//! item 2; roadmap M5, `git-remote-afs`).
//!
...
//! (fetch/clone) or `connect git-receive-pack` (push). We materialize the afs
//! history into a throwaway real git repository with the M5 object codec
//! ([`afs_git::export_git`]), hand our stdin/stdout to a genuine
...
//! into afs ([`afs_git::import_git`]).
//!
//! A URL is `afs://<workspace-path>`, where the workspace is an afs directory
//! (holding `meta.db` + `cas/`), the same one the `afs` CLI's `--workspace`
//! points at.
//!
...
fn main() {
    if let Err(e) = run() {
        eprintln!("git-remote-afs: {e:#}");
        std::process::exit(1);
    }
}

fn run() -> Result<()> {
    // git invokes us as `git-remote-afs <remote> <url>`; the URL is last.
    ...
    let ws_path = url
        ...
        .to_string();
    if ws_path.is_empty() {
        bail!("usage: git-remote-afs <remote> afs://<workspace>");
    }
...
fn handle_connect(rt: &tokio::runtime::Runtime, ws_path: &str, service: &str) -> Result<i32> {
    let ws = rt
        ...
        .with_context(|| format!("opening afs workspace at {ws_path}"))?;
    ...
    // Materialize the current afs history into a real git repo (SHA-1, what
    ...
    let code = match service {
        ...
        "git-receive-pack" => {
            ...
            // Import every branch the push created or moved back into afs.
