//! Export an afs branch into a real on-disk git repository (`docs/DESIGN.md`
//! §4c, interop items 1 & 3).
//!
//! Walks the afs commit DAG from a branch head and re-encodes every commit,
...
use afs_core::objectgraph::TreeKind;
...
pub async fn export_git(ws: &Workspace, dir: &Path, opts: &ExportOptions) -> Result<GitExport> {
    let branch = match &opts.branch {
        ...
        None => ws
            .current_branch()
            .await?
            .ok_or_else(|| AfsError::InvalidArgument("HEAD is detached; pass a branch".into()))?,
    };
    let head = ws.fs().branch_head(&branch).await?.ok_or_else(|| {
        AfsError::InvalidArgument(format!("branch {branch} has no commits to export"))
    })?;
...
struct Exporter<'a> {
    ...
    /// afs tree hash -> git tree oid hex.
    trees: HashMap<Hash, String>,
    /// afs commit hash -> git commit oid hex.
...
impl Exporter<'_> {
    #[async_recursion]
    async fn export_commit(&mut self, afs_hash: Hash) -> Result<String> {
        if let Some(oid) = self.commits.get(&afs_hash) {
            return Ok(oid.clone());
        }
        let commit = self.ws.fs().commit_object(&afs_hash).await?;
        ...
        self.commits.insert(afs_hash, obj.oid_hex.clone());
        Ok(obj.oid_hex)
    }

    #[async_recursion]
    async fn export_tree(&mut self, afs_hash: Hash) -> Result<String> {
        if let Some(oid) = self.trees.get(&afs_hash) {
            return Ok(oid.clone());
        }
        let tree = self.ws.fs().tree_object(&afs_hash).await?;
        ...
        write_loose(&self.git_dir, &obj)?;
        self.trees.insert(afs_hash, obj.oid_hex.clone());
        Ok(obj.oid_hex)
    }
