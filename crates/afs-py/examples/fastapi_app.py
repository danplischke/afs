"""Example: driving an afs workspace from FastAPI.
...
write to that user/agent via `WriteCtx`. afs records blame + an edit-op audit
...
WS: afs.Workspace  # set on startup


@asynccontextmanager
async def lifespan(app: FastAPI):
    global WS
    # Local dev; swap for afs.Workspace.open_pg(dsn, cas_dir) for multi-writer.
...
# --- identity injection -----------------------------------------------------
# Replace this with your real auth. Resolve the request's principal to an afs
...
async def write_file(path: str, body: bytes, ctx: afs.WriteCtx = Depends(current_ctx)):
...
async def suggest(path: str, body: bytes, summary: Optional[str] = None,
                  ctx: afs.WriteCtx = Depends(current_ctx)):
...
async def accept(sid: int, ctx: afs.WriteCtx = Depends(current_ctx)):
    try:
        await WS.accept_suggestion(sid, ctx)  # applied, credited to the agent
    except afs.ConflictError as e:
...
#
#   mount = WS.mount("/mnt/afs")          # FUSE, returns a handle
#   ...
#   mount.unmount()
#
#   import asyncio
