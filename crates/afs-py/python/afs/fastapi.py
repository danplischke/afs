"""A ready-made FastAPI :class:`~fastapi.APIRouter` over an afs workspace.

afs deliberately has no built-in authentication: an attributed write is only as
...
to.
    ...
    async def authn(authorization: str = Header(...)) -> afs.WriteCtx:
        actor_id, session_id = await my_auth.resolve(authorization)   # your logic
        if actor_id is None:
            raise HTTPException(401, "unauthenticated")
        return afs.WriteCtx.session(actor_id, session_id)
...
:class:`~afs.WriteCtx` it returns — so blame and the audit log reflect the
...
Requires FastAPI: ``pip install "afs[fastapi]"``.
"""
...
try:
    ...
    raise ImportError(
        "afs.fastapi requires FastAPI. Install it with: pip install \"afs[fastapi]\""
    ) from exc
...
_AFS_EXC: Optional[tuple] = None


def _afs_exc() -> tuple:
    """(ConflictError, AfsError), resolved lazily so this module imports without
    ...
    if _AFS_EXC is None:
        try:
            ...
            _AFS_EXC = (afs.ConflictError, afs.AfsError)
        except Exception:  # pragma: no cover - only if the native module is absent

            class _Never(Exception):
                pass

            _AFS_EXC = (_Never, _Never)
    return _AFS_EXC


async def _run(awaitable: Awaitable[Any]) -> Any:
    """Await a workspace call, mapping afs errors to HTTP status codes."""
    conflict_error, afs_error = _afs_exc()
    try:
...
def build_router(
    ...
    **router_kwargs: Any,
    ...
    ----------
    ws:
        An open :class:`afs.Workspace` (or any object with the same async
        methods — handy for testing).
    authn:
        Your authentication dependency. It must resolve the request to the
        :class:`afs.WriteCtx` the change should be attributed to (raise
