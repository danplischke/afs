...
def test_suggestion_content():
    async def _exercise():
        ...
        await ws.write_as(afs.WriteCtx.actor(dan), "/doc.md", b"one\ntwo\n")
        sid = await ws.suggest(
            afs.WriteCtx.session(claude, sc), "/doc.md", b"one\ntwo\nthree\n", "add a line"
        )
        ...
        did = await ws.suggest_delete(afs.WriteCtx.session(claude, sc), "/doc.md", "remove it")
