"""End-to-end test of the afs Python bindings.

Build + run:
    maturin develop            # from crates/afs-py, in a venv
...
async def _exercise():
    d = tempfile.mkdtemp()
    ws = await afs.Workspace.open_local(os.path.join(d, "meta.db"), os.path.join(d, "cas"))
    ...
    ctx = afs.WriteCtx.session(agent, sess)
    ...
    await ws.accept_suggestion(sid, afs.WriteCtx.actor(human))
    assert bytes(await ws.read("/notes.txt")) == b"line one\nline 2!\n"
    ...
    try:
        await ws.accept_suggestion(sid2, afs.WriteCtx.actor(human))
        raise AssertionError("expected ConflictError")
    except afs.ConflictError:
        pass
...
if __name__ == "__main__":
    asyncio.run(_exercise())
    print("OK  fuse_mountable() =", afs.fuse_mountable())
