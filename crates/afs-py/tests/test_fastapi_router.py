"""Tests for afs.fastapi.build_router.

Two layers:
  * unit tests against a fake workspace — auth enforcement, forge-prevention,
    error mapping — with no I/O;
  * an integration test against a real afs.Workspace, proving an attributed
...
class FakeWs:
    ...
    async def accept_suggestion(self, sid, ctx):
        ...
        if sid == 999:
            raise afs.ConflictError("stale base")
...
def test_integration_attribution_end_to_end():
    ...
    async def _setup():
        # The afs awaitables bind to the running loop, so create them inside one.
