...
The Postgres part self-skips unless AFS_PG_TEST_URL is set, e.g.
    AFS_PG_TEST_URL="host=127.0.0.1 port=5433 user=postgres password=postgres dbname=afs"
...
def test_subscribe_pushes_events():
    dsn = os.environ.get("AFS_PG_TEST_URL")
    if not dsn:
        print("skip test_subscribe_pushes_events: AFS_PG_TEST_URL unset")
        return

    async def _exercise():
        d = tempfile.mkdtemp()
        ws = await afs.Workspace.open_pg(dsn, os.path.join(d, "cas"))
