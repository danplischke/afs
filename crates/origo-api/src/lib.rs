//! origo-api — an HTTP/JSON surface over a workspace (`docs/DESIGN.md` §6, M7).
//!
//! A thin [`axum`] layer that exposes the same operations as the `origo` CLI to any
...
//! goes through [`origo_sdk::Workspace`], so writes are recorded on the change feed
...
//! for reads too. This mirrors the Python `origo.fastapi.build_router` model.
//!
...
use origo_sdk::{Workspace, WriteCtx};
...
/// The authenticated identity behind a request: the origo actor a mutation is
/// attributed to, plus an optional session. origo never trusts a client-named
...
/// map it to the origo actor it should be attributed to. Return `None` to reject
/// the request with `401`. This is the Rust counterpart to the `authn`
/// dependency of the Python `origo.fastapi.build_router`.
...
/// For local single-user dev only — the CLI uses it when `origo serve` runs on a
...
/// Normalize a URL-tail path to an absolute origo path.
...
/// An HTTP error: either a mapped [`origo_sdk::OrigoError`] or an explicit status
/// (e.g. `401` from the [`Auth`] extractor).
enum ApiError {
    Origo(origo_sdk::OrigoError),
...
impl From<origo_sdk::OrigoError> for ApiError {
    fn from(e: origo_sdk::OrigoError) -> Self {
        ApiError::Origo(e)
    }
}
    ...
    let out = ws
        ...
        .map(|d| DiffEntryDto {
            ...
            status: match d.status {
                origo_sdk::DiffStatus::Added => "added",
                origo_sdk::DiffStatus::Modified => "modified",
                origo_sdk::DiffStatus::Deleted => "deleted",
            },
        })
        .collect();
    Ok(Json(out))
...
impl From<origo_sdk::Suggestion> for SuggestionDto {
    fn from(s: origo_sdk::Suggestion) -> Self {
        Self {
            id: s.id,
            actor_id: s.actor_id,
...
async fn list_suggestions(
    ...
    let status = match q.status.as_deref() {
        Some(s) => Some(
            origo_sdk::SuggestionStatus::parse(s)
                .ok_or_else(|| origo_sdk::OrigoError::InvalidArgument(format!("bad status {s}")))?,
        ),
        None => None,
    };
    let out = ws
...
async fn get_suggestion(
    ...
    let s = ws
        .get_suggestion(id)
        .await?
        .ok_or_else(|| origo_sdk::OrigoError::NotFound(format!("suggestion #{id}")))?;
    Ok(Json(s.into()))
}
