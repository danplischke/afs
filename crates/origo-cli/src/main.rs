//! origo — a minimal CLI over an origo workspace (M0).
//!
//! Enough to exercise the engine from a shell:
//!
//! ```text
//! origo --workspace ./ws init
//! echo hello | origo --workspace ./ws write /notes/a.txt
...
use origo_sdk::{MergeOutcome, SuggestionStatus, Workspace, WriteCtx};
...
enum Cmd {
    ...
    /// `--path`. E.g. `origo diff main feature` or `origo diff main feature --path /x`.
    Diff {
        from: String,
        to: String,
    ...
    /// for untrusted code). Usage: `origo sandbox --actor 1 -- <cmd> [args...]`
    Sandbox {
    ...
    /// in a fast unprivileged kernel overlay while its changes stream into origo
    ...
    /// bubblewrap boundary. Usage: `origo overlay --actor 1 -- <cmd>`
    Overlay {
        /// Attribute the agent's changes to this actor id.
        #[arg(long)]
        actor: Option<i64>,
        /// How often (ms) to sync the agent's changes into origo while it runs.
...
async fn main() -> Result<()> {
    ...
    // Opt into encryption at rest by setting ORIGO_ENCRYPTION_KEY (kept out of
    // argv/history); the same value must be used every time for this workspace.
    let ws = match std::env::var("ORIGO_ENCRYPTION_KEY") {
    ...
    match cli.cmd {
        Cmd::Init => {
            println!("initialized origo workspace at {}", cli.workspace.display());
        }
        Cmd::Mkdir { path } => {
        ...
        Cmd::Sandbox {
            ...
            if isolate {
                if !origo_sandbox::bwrap_available() {
                    anyhow::bail!(
                        "--isolate needs bubblewrap (`bwrap`) on PATH (>= 0.8.0, for overlay support)"
                    );
                }
            } else if !origo_sandbox::overlay_supported() {
                anyhow::bail!(
            ...
            let opts = origo_sandbox::RunOpts {
                actor,
                discard,
                work_root: tmp.clone(),
                isolate,
            };
            let outcome = origo_sandbox::run(&ws, opts, &cmd).await?;
        ...
        Cmd::Overlay {
            ...
            let opts = origo_sandbox::LiveOpts {
                actor,
                work_root: tmp.clone(),
                sync_interval: std::time::Duration::from_millis(sync_ms),
                isolate,
            };
            let outcome = origo_sandbox::run_live(&ws, opts, &cmd).await?;
            let _ = std::fs::remove_dir_all(&tmp);
            println!(
                "agent exited {}; streamed {} change(s) into origo",
                outcome.exit_code, outcome.files_changed
            );
            std::process::exit(outcome.exit_code);
        }
        Cmd::Mount { mountpoint } => {
            if !origo_fuse::mountable() {
                anyhow::bail!("FUSE mount unavailable here (needs root + /dev/fuse)");
            }
            std::fs::create_dir_all(&mountpoint)?;
            println!(
                "mounting origo at {} (unmount with `umount` to stop)",
                mountpoint.display()
            );
            // The mount drives its own runtime, so run it off the async main thread.
            let handle = std::thread::spawn(move || origo_fuse::mount(ws, &mountpoint));
            handle
                .join()
                .map_err(|_| anyhow::anyhow!("mount thread panicked"))??;
        }
        Cmd::Mcp { agent_name, model } => {
            let server = origo_mcp::McpServer::create(ws, &agent_name, &model).await?;
        ...
        Cmd::Git { cmd } => match cmd {
            GitCmd::Export {
                ...
                let format = origo_git::ObjectFormat::parse(&format)
                    .ok_or_else(|| anyhow::anyhow!("format must be `sha1` or `sha256`"))?;
                let opts = origo_git::ExportOptions {
                    format,
                    branch,
                    lfs_threshold,
                };
                let out = origo_git::export_git(&ws, &dir, &opts).await?;
                println!(
            ...
            GitCmd::Import { dir, branch } => {
                let head = origo_git::import_git(&ws, &dir, &branch).await?;
                println!(
        ...
        Cmd::Serve { addr, auth_tokens } => {
            ...
            println!("serving origo at http://{addr} (Ctrl-C to stop)");
            origo_api::serve(std::sync::Arc::new(ws), addr, auth).await?;
        }
        Cmd::Nfs { addr } => {
            ...
            println!(
                "serving origo over NFSv3 at {addr}\n  mount with: mount -t nfs -o vers=3,tcp,port=<port>,mountport=<port>,nolock <host>:/ /mnt"
            );
            origo_nfs::serve(ws, &addr).await?;
        }
    }
    Ok(())
}

/// Build the HTTP API authenticator from `--auth-token` specs. origo never trusts a
...
async fn build_api_auth(
    ...
    addr: &std::net::SocketAddr,
    specs: &[String],
) -> Result<std::sync::Arc<dyn origo_api::Authenticator>> {
    if specs.is_empty() {
        ...
        return Ok(std::sync::Arc::new(origo_api::LocalDevAuth(
            origo_api::Principal {
                actor,
                session: None,
            },
    ...
    let mut bearer = origo_api::BearerAuth::new();
    for spec in specs {
