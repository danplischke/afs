...
impl<M: MetadataStore, C: ContentStore> Fs<M, C> {
    ...
    /// is how an application binds its own user id to an origo actor without keeping
    ...
    pub async fn find_or_create_actor(&self, init: ActorInit) -> Result<i64> {
        let subject = init.auth_subject.clone().ok_or_else(|| {
            OrigoError::InvalidArgument("find_or_create_actor requires auth_subject".into())
        })?;
    ...
    /// be absent/empty"), returning [`OrigoError::Conflict`] otherwise. The check
    ...
    pub async fn blame(&self, path: &str) -> Result<Vec<BlameRange>> {
        ...
        for r in &map.runs {
            let actor = self
                ...
                .get_actor(r.actor)
                .await?
                .ok_or_else(|| OrigoError::NotFound(format!("actor {}", r.actor)))?;
            out.push(BlameRange {
                line_start: line,
