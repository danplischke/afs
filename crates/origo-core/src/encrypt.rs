...
//! salt is not secret but must persist; origo-sdk keeps it alongside the content
...
const NONCE_CONTEXT: &str = "origo content nonce v1";
...
impl EncryptedStore {
    ...
    /// must be persisted somewhere durable that travels with the store — origo-sdk
    ...
    pub fn from_passphrase(
        ...
        argon
            ...
            .map_err(|e| OrigoError::Content(format!("passphrase key derivation failed: {e}")))?;
    ...
    fn encrypt(&self, storage_key: &Hash, plaintext: &[u8]) -> Result<Vec<u8>> {
        self.cipher
            ...
            .map_err(|_| OrigoError::Content("encryption failed".into()))
    }
...
impl ContentStore for EncryptedStore {
    ...
    async fn get_range(&self, hash: &Hash, off: u64, len: u64) -> Result<Bytes> {
        // AEAD authenticates the whole object, so decrypt then slice. origo objects
