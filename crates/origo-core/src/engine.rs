...
pub(crate) fn validate_component(name: &str) -> Result<()> {
    if name.is_empty() || name == "." || name == ".." || name.contains('/') || name.contains('\0') {
        return Err(OrigoError::InvalidPath(format!(
...
impl<M: MetadataStore, C: ContentStore> Fs<M, C> {
    ...
    pub(crate) async fn resolve_parent<'a>(&self, path: &'a str) -> Result<(Ino, &'a str)> {
        ...
        let (name, dirs) = segs
            .split_last()
            .ok_or_else(|| OrigoError::InvalidPath(format!("no basename in {path}")))?;
        let mut ino = INO_ROOT;
        for &seg in dirs {
            ino = self
                .meta
    ...
    pub(crate) async fn ensure_dir(&self, ino: Ino) -> Result<()> {
        let inode = self
            ...
            .get_inode(ino)
            .await?
            .ok_or_else(|| OrigoError::NotFound(format!("ino {ino}")))?;
        if inode.kind != FileKind::Dir {
            return Err(OrigoError::NotADirectory(format!("ino {ino}")));
        }
        Ok(())
    }
    ...
    pub async fn mkdir(&self, path: &str) -> Result<Ino> {
        ...
        if self.meta.lookup(parent, name).await?.is_some() {
            return Err(OrigoError::AlreadyExists(path.to_string()));
        }
            ...
            match self.meta.lookup(ino, seg).await? {
                Some(child) => {
                    let inode = self
                        ...
                        .ok_or_else(|| OrigoError::NotFound(path.to_string()))?;
                    if inode.kind != FileKind::Dir {
                        return Err(OrigoError::NotADirectory(path.to_string()));
                    }
                    ino = child;
                }
                None => {
                    ...
                    match tx.add_dentry(ino, seg, child).await {
                        ...
                        Err(OrigoError::AlreadyExists(_)) => {
    ...
    pub async fn rmdir(&self, path: &str) -> Result<()> {
        ...
        if self.meta.child_count(ino).await? > 0 {
            return Err(OrigoError::DirectoryNotEmpty(path.to_string()));
        }
        ...
        match self.meta.lookup(parent, name).await? {
            Some(existing) => {
                let inode = self
                    ...
                    .ok_or_else(|| OrigoError::NotFound(path.to_string()))?;
                if inode.kind == FileKind::Dir {
                    return Err(OrigoError::IsADirectory(path.to_string()));
                }
                Ok(Some(existing))
            }
            None => Ok(None),
    ...
    pub async fn write(&self, path: &str, data: &[u8]) -> Result<()> {
        ...
        Err(OrigoError::Conflict(format!(
        ...
        R: std::io::Read + Send + 'static,
    {
        ...
        while let Some(item) = rx.recv().await {
            let data = item.map_err(OrigoError::Content)?;
            size += data.len() as u64;
    ...
    pub async fn read(&self, path: &str) -> Result<Bytes> {
        ...
        let inode = self
            ...
            .ok_or_else(|| OrigoError::NotFound(path.to_string()))?;
        match inode.kind {
            FileKind::Dir => Err(OrigoError::IsADirectory(path.to_string())),
            FileKind::Symlink => Err(OrigoError::InvalidArgument(format!("{path} is a symlink"))),
    ...
    async fn open_for_stream(&self, path: &str) -> Result<Option<Manifest>> {
        ...
        let inode = self
            ...
            .ok_or_else(|| OrigoError::NotFound(path.to_string()))?;
        match inode.kind {
            FileKind::Dir => return Err(OrigoError::IsADirectory(path.to_string())),
            FileKind::Symlink => {
                return Err(OrigoError::InvalidArgument(format!("{path} is a symlink")));
            }
            FileKind::File => {}
        }
        match inode.content {
    ...
    /// memory (there is no fixed size ceiling on origo files).
    ///
    ...
    pub async fn rename(&self, from: &str, to: &str) -> Result<()> {
        ...
        let sino = self
            ...
            .lookup(sp, sn)
            .await?
            .ok_or_else(|| OrigoError::NotFound(from.to_string()))?;
        ...
        let overwrite = match self.meta.lookup(dp, dn).await? {
            ...
            Some(dino) => {
                let dinode = self
                    ...
                    .get_inode(dino)
                    .await?
                    .ok_or_else(|| OrigoError::NotFound(to.to_string()))?;
                if dinode.kind == FileKind::Dir && self.meta.child_count(dino).await? > 0 {
                    return Err(OrigoError::DirectoryNotEmpty(to.to_string()));
                }
                Some((dino, dinode))
            }
            None => None,
    ...
    pub async fn symlink(&self, target: &str, linkpath: &str) -> Result<Ino> {
        ...
        self.ensure_dir(parent).await?;
        if self.meta.lookup(parent, name).await?.is_some() {
            return Err(OrigoError::AlreadyExists(linkpath.to_string()));
        }
    ...
    pub async fn readlink(&self, path: &str) -> Result<String> {
        ...
        self.meta
            .get_symlink(ino)
            .await?
            .ok_or_else(|| OrigoError::InvalidArgument(format!("{path} is not a symlink")))
    }
}
