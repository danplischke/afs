...
impl Tree {
    ...
    pub fn decode(bytes: &[u8]) -> Result<Tree> {
        let bad = || OrigoError::Content("malformed tree object".to_string());
...
impl Commit {
    ...
    pub fn decode(bytes: &[u8]) -> Result<Commit> {
        let bad = || OrigoError::Content("malformed commit object".to_string());
...
impl RefSnapshot {
    ...
    pub fn decode(bytes: &[u8]) -> Result<RefSnapshot> {
        let bad = || OrigoError::Content("malformed ref snapshot".to_string());
...
pub enum VersioningMode {
    ...
    Off,
    /// origo's native content-addressed commit DAG.
    Native,
    /// Native commit DAG plus git interop: the `origo-git` bridge exports/imports
