...
impl PackLoc {
    ...
    fn decode(b: &[u8]) -> Result<Self> {
        if b.len() != 40 {
            return Err(OrigoError::Content("malformed pack index entry".into()));
        }
        let mut pack = [0u8; 32];
...
impl PackStore {
    ...
    async fn locate(&self, hash: &Hash) -> Result<Option<PackLoc>> {
        match self.index.get(hash).await {
            ...
            Err(OrigoError::ContentMissing(_)) | Err(OrigoError::NotFound(_)) => Ok(None),
            Err(e) => Err(e),
        }
    }
...
fn parse_trailer(pack: &[u8]) -> Result<Vec<(Hash, u32, u32)>> {
    let bad = || OrigoError::Content("malformed pack trailer".into());
    if pack.len() < 4 {
        return Err(bad());
    }
...
impl ContentStore for PackStore {
    ...
    async fn get(&self, hash: &Hash) -> Result<Bytes> {
        ...
        match self.locate(hash).await? {
            ...
            None => Err(OrigoError::ContentMissing(hash.to_hex())),
        }
    }
