...
impl PostgresMetadataStore {
    ...
    /// `postgres://user:pass@host/db` or `host=/var/run/postgresql dbname=origo`.
    pub async fn connect(dsn: &str) -> Result<Self> {
        let cfg: tokio_postgres::Config = dsn
            .parse()
            .map_err(|e: tokio_postgres::Error| OrigoError::Metadata(e.to_string()))?;
        ...
        let pool = Pool::builder(mgr)
            ...
            .build()
            .map_err(|e| OrigoError::Metadata(e.to_string()))?;
        Ok(Self {
            pool,
            dsn: dsn.to_string(),
        })
...
fn row_to_inode(r: &Row) -> Result<Inode> {
    ...
    let kind = FileKind::parse(&kind_s)
        .ok_or_else(|| OrigoError::Metadata(format!("unknown inode kind {kind_s:?}")))?;
    let content = match r.get::<_, Option<String>>(5) {
        Some(s) => Some(
            Hash::from_hex(&s)
                .ok_or_else(|| OrigoError::Metadata(format!("bad content hash {s:?}")))?,
        ),
        None => None,
    };
    Ok(Inode {
    ...
    async fn add_dentry(&self, parent: Ino, name: &str, ino: Ino) -> Result<()> {
            ...
            .await
        {
            ...
            Err(e) if e.code() == Some(&SqlState::UNIQUE_VIOLATION) => {
                Err(OrigoError::AlreadyExists(name.to_string()))
            }
            Err(e) => Err(e.into()),
        }
    }
        ...
        match row {
            Some(r) => {
                ...
                let kind = ActorKind::parse(&kind_s)
                    .ok_or_else(|| OrigoError::Metadata(format!("bad actor kind {kind_s:?}")))?;
                Ok(Some(Actor {
                    id: r.get(0),
                    kind,
