...
impl<M: MetadataStore, C: ContentStore> Fs<M, C> {
    ...
    pub(crate) async fn ensure_commits_enabled(&self) -> Result<()> {
        if !self.versioning_mode().await?.commits_enabled() {
            return Err(OrigoError::InvalidArgument(
    ...
    pub async fn commit(&self, author: &str, message: &str) -> Result<Hash> {
        ...
        let branch = self.current_branch().await?.ok_or_else(|| {
            OrigoError::InvalidArgument("cannot commit with a detached HEAD".into())
        })?;
        ...
        let swapped = self
            ...
            .await?;
        if !swapped {
            return Err(OrigoError::Metadata(
    ...
    async fn build_tree(&self, dir_ino: Ino) -> Result<Hash> {
        ...
        for de in self.meta.list_dir(dir_ino).await? {
            let inode = self
                ...
                .get_inode(de.ino)
                .await?
                .ok_or_else(|| OrigoError::NotFound(format!("ino {}", de.ino)))?;
    ...
    pub async fn create_branch(&self, name: &str) -> Result<()> {
        ...
        let head = self.head_commit().await?.ok_or_else(|| {
            OrigoError::InvalidArgument("cannot branch before the first commit".into())
        })?;
        if !self.meta.cas_ref(name, None, &head.to_hex()).await? {
            return Err(OrigoError::AlreadyExists(format!("branch {name}")));
        }
        self.mirror_refs().await?;
        Ok(())
    }
    ...
    pub async fn checkout(&self, branch: &str) -> Result<()> {
        ...
        let value = self
            ...
            .get_ref(branch)
            .await?
            .ok_or_else(|| OrigoError::NotFound(format!("branch {branch}")))?;
        let commit_hash =
            Hash::from_hex(&value).ok_or_else(|| OrigoError::Metadata("bad ref value".into()))?;
    ...
    pub async fn resolve_commit(&self, name: &str) -> Result<Hash> {
        if let Some(v) = self.meta.get_ref(name).await? {
            ...
            return Hash::from_hex(&target)
                .ok_or_else(|| OrigoError::Metadata("bad ref value".into()));
        }
        Hash::from_hex(name).ok_or_else(|| OrigoError::NotFound(format!("ref or commit {name}")))
    }
            ...
            match de.kind {
                ...
                FileKind::File => {
                    let inode = self
                        ...
                        .get_inode(de.ino)
                        .await?
                        .ok_or_else(|| OrigoError::NotFound(path.clone()))?;
                    let h = match inode.content {
                        Some(h) => h,
