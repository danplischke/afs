...
impl<M: MetadataStore, C: ContentStore> Fs<M, C> {
    ...
    pub async fn vfs_write(&self, ino: Ino, offset: u64, data: &[u8]) -> Result<u32> {
        ...
        let end = offset
            ...
            .ok_or_else(|| OrigoError::TooLarge(format!("write end overflows u64 (ino {ino})")))?;
        let end = usize::try_from(end)
            .map_err(|_| OrigoError::TooLarge(format!("write past {end} bytes (ino {ino})")))?;
        if bytes.len() < end {
            let extra = end - bytes.len();
            bytes.try_reserve(extra).map_err(|_| {
                OrigoError::TooLarge(format!("cannot allocate {end} bytes (ino {ino})"))
            })?;
            bytes.resize(end, 0);
        }
    ...
    pub async fn vfs_truncate(&self, ino: Ino, size: u64) -> Result<()> {
        ...
        let target = usize::try_from(size)
            .map_err(|_| OrigoError::TooLarge(format!("truncate to {size} bytes (ino {ino})")))?;
        let mut bytes = match inode.content {
            Some(h) => self.read_body(&h).await?,
            None => Vec::new(),
        };
        if target > bytes.len() {
            ...
            bytes.try_reserve(extra).map_err(|_| {
                OrigoError::TooLarge(format!("cannot allocate {size} bytes (ino {ino})"))
            })?;
        }
    ...
    pub async fn vfs_create(&self, parent: Ino, name: &str, mode: u32) -> Result<Inode> {
        ...
        if self.meta.lookup(parent, name).await?.is_some() {
            return Err(OrigoError::AlreadyExists(name.to_string()));
        }
    ...
    pub async fn vfs_unlink(&self, parent: Ino, name: &str) -> Result<()> {
        let ino = self
            ...
            .lookup(parent, name)
            .await?
            .ok_or_else(|| OrigoError::NotFound(name.to_string()))?;
        let inode = self.vfs_getattr(ino).await?;
        if inode.kind == FileKind::Dir {
            return Err(OrigoError::IsADirectory(name.to_string()));
        }
        let mut tx = self.meta.begin().await?;
    ...
    pub async fn vfs_rmdir(&self, parent: Ino, name: &str) -> Result<()> {
        ...
        let inode = self.vfs_getattr(ino).await?;
        if inode.kind != FileKind::Dir {
            return Err(OrigoError::NotADirectory(name.to_string()));
        }
        if self.meta.child_count(ino).await? > 0 {
            return Err(OrigoError::DirectoryNotEmpty(name.to_string()));
        }
        let mut tx = self.meta.begin().await?;
        tx.remove_dentry(parent, name).await?;
        tx.delete_inode(ino).await?;
        ...
        let overwrite = match self.meta.lookup(newparent, newname).await? {
            ...
            Some(dino) => {
                ...
                if dinode.kind == FileKind::Dir && self.meta.child_count(dino).await? > 0 {
                    return Err(OrigoError::DirectoryNotEmpty(newname.to_string()));
                }
                Some((dino, dinode))
            }
            None => None,
    ...
    pub async fn vfs_readlink(&self, ino: Ino) -> Result<String> {
        self.meta
            .get_symlink(ino)
            .await?
            .ok_or_else(|| OrigoError::InvalidArgument(format!("ino {ino} is not a symlink")))
    }
}
