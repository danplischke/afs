...
use origo_core::{ActorInit, EventInit, MetadataStore, SqliteMetadataStore};
...
/// Against a live Postgres, appending an event must fire `NOTIFY origo_events`
/// with the new seq — so consumers can be pushed instead of polling.
/// Self-skips unless `ORIGO_PG_TEST_URL` is set.
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn postgres_append_fires_notify() {
    use futures::{StreamExt, future, stream};
    use origo_core::{EVENT_CHANNEL, PostgresMetadataStore};
    use std::time::Duration;

    let Ok(dsn) = std::env::var("ORIGO_PG_TEST_URL") else {
        eprintln!("skipping postgres_append_fires_notify: ORIGO_PG_TEST_URL unset");
        return;
    };
