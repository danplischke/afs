...
use origo_core::{ContentStore, Hash, LocalCasStore, MemStore, ObjectContentStore, TieredStore};
use std::sync::Arc;
...
///   ORIGO_S3_TEST_BUCKET, ORIGO_S3_TEST_REGION, ORIGO_S3_TEST_ENDPOINT,
///   ORIGO_S3_TEST_ACCESS_KEY_ID, ORIGO_S3_TEST_SECRET_ACCESS_KEY
#[tokio::test]
#[ignore = "requires an S3-compatible endpoint; set ORIGO_S3_TEST_* to run"]
async fn s3_backend() {
    use origo_core::S3Config;
    let bucket = std::env::var("ORIGO_S3_TEST_BUCKET").expect("ORIGO_S3_TEST_BUCKET");
    let cfg = S3Config {
        bucket,
        region: std::env::var("ORIGO_S3_TEST_REGION").unwrap_or_else(|_| "us-east-1".into()),
        endpoint: std::env::var("ORIGO_S3_TEST_ENDPOINT").ok(),
        allow_http: true,
        access_key_id: std::env::var("ORIGO_S3_TEST_ACCESS_KEY_ID").ok(),
        secret_access_key: std::env::var("ORIGO_S3_TEST_SECRET_ACCESS_KEY").ok(),
        prefix: Some("origo-test".into()),
    };
...
/// `fake-gcs-server`):
///   ORIGO_GCS_TEST_BUCKET (required),
///   ORIGO_GCS_TEST_SERVICE_ACCOUNT_PATH   (JSON key file; for fake-gcs-server, point
///     its `gcs_base_url` at the emulator and set `disable_oauth: true`),
///   ORIGO_GCS_TEST_SERVICE_ACCOUNT_KEY    (inline JSON; alternative to the path),
///   ORIGO_GCS_TEST_APPLICATION_CREDENTIALS (ADC file).
/// With none of the credential vars set, ADC / the metadata server are used.
#[tokio::test]
#[ignore = "requires a GCS bucket or emulator; set ORIGO_GCS_TEST_* to run"]
async fn gcs_backend() {
    use origo_core::GcsConfig;
    let bucket = std::env::var("ORIGO_GCS_TEST_BUCKET").expect("ORIGO_GCS_TEST_BUCKET");
    let cfg = GcsConfig {
        bucket,
        service_account_path: std::env::var("ORIGO_GCS_TEST_SERVICE_ACCOUNT_PATH").ok(),
        service_account_key: std::env::var("ORIGO_GCS_TEST_SERVICE_ACCOUNT_KEY").ok(),
        application_credentials: std::env::var("ORIGO_GCS_TEST_APPLICATION_CREDENTIALS").ok(),
        prefix: Some("origo-test".into()),
    };
