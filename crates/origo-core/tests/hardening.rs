...
use origo_core::{
    ActorInit, ChunkRef, Fs, Hash, INO_ROOT, Manifest, MemStore, MetadataStore, OrigoError,
...
async fn traversal_path_components_are_rejected_everywhere() {
    ...
    // The path API (origo-api / MCP / SDK / CLI all funnel through `split`).
    for bad in ["/a/../b", "/../etc/passwd", "/./x", "/a/./b"] {
        assert!(
            matches!(fs.mkdir_p(bad).await, Err(OrigoError::InvalidPath(_))),
            "mkdir_p should reject {bad:?}"
        );
    }
    assert!(matches!(
        fs.write("/x/../y", b"z").await,
        Err(OrigoError::InvalidPath(_))
    ));
...
fn objectgraph_decoders_reject_hostile_counts_without_oom() {
    use origo_core::{Commit, RefSnapshot, Tree};
...
async fn concurrent_merges_never_orphan_a_success() {
    ...
    for o in &outcomes {
        if let Ok(origo_core::MergeOutcome::Merged(h))
        | Ok(origo_core::MergeOutcome::FastForward(h)) = o
        {
            assert_eq!(
                Some(*h),
                head,
...
async fn checkout_rolls_back_and_keeps_the_tree_on_a_missing_object() {
    use origo_core::{Commit, ContentStore, Tree, TreeEntry, TreeKind};
    let fs = fixture().await;
