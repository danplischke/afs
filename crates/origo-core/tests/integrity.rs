...
//! corruption surfaces as `OrigoError::Corrupt` instead of silently propagating.

use origo_core::{
    ContentStore, EncryptedStore, LocalCasStore, OrigoError, PackStore, VerifyingStore,
};
use std::sync::Arc;
...
async fn verifying_store_detects_a_flipped_byte() {
    ...
    assert!(
        matches!(verified.get(&h).await, Err(OrigoError::Corrupt(_))),
...
async fn repack_refuses_to_relaunder_a_corrupt_chunk() {
    ...
    assert!(
        matches!(pack.repack().await, Err(OrigoError::Corrupt(_))),
