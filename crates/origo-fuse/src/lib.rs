//! origo-fuse — mount an origo workspace as a POSIX filesystem via FUSE
//! (`docs/DESIGN.md` §4e).
//!
//! A [`fuser::Filesystem`] adapter over the inode-oriented [`Fs::vfs_*`] methods.
//! origo is async and FUSE callbacks are synchronous, so each callback drives the
...
use origo_sdk::{FileKind, Inode, OrigoError, Workspace};
use std::ffi::OsStr;
...
/// A FUSE filesystem backed by an origo [`Workspace`].
pub struct OrigoFuse {
    ws: Workspace,
    rt: Runtime,
}
...
fn config() -> Config {
    ...
    config.mount_options = vec![
        MountOption::FSName("origo".to_string()),
...
fn errno(e: &OrigoError) -> Errno {
    match e {
        OrigoError::NotFound(_) => Errno::ENOENT,
        OrigoError::AlreadyExists(_) => Errno::EEXIST,
        OrigoError::DirectoryNotEmpty(_) => Errno::ENOTEMPTY,
        OrigoError::IsADirectory(_) => Errno::EISDIR,
        OrigoError::NotADirectory(_) => Errno::ENOTDIR,
        OrigoError::InvalidArgument(_) | OrigoError::InvalidPath(_) => Errno::EINVAL,
        _ => Errno::EIO,
    }
}
