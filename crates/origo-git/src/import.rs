//! Import a real git repository's history into an origo workspace (`docs/DESIGN.md`
//! §4c, interop item 1, reverse direction).
//!
//! Reads loose objects from `<dir>/.git` (or a bare `<dir>`), translating each
//! git commit/tree/blob into origo commits/trees/blobs, then points the target
...
use origo_core::objectgraph::{Commit, Tree, TreeEntry, TreeKind};
...
/// check it out. Returns the origo commit hash of the imported head.
pub async fn import_git(ws: &Workspace, dir: &Path, branch: &str) -> Result<Hash> {
    ...
    let fmt = ObjectFormat::from_hex_len(head_hex.len())
        .ok_or_else(|| OrigoError::Content(format!("unrecognized object id: {head_hex}")))?;
...
fn resolve_head(git_dir: &Path, branch: &str) -> Result<String> {
    ...
    Err(OrigoError::NotFound(format!("branch {branch}")))
}
...
struct Importer<'a> {
    ...
    git_dir: PathBuf,
    fmt: ObjectFormat,
    /// git commit oid hex -> origo commit hash.
    commits: HashMap<String, Hash>,
    /// git tree oid hex -> origo tree hash.
    ...
    fn read_blob(&self, oid_hex: &str) -> Result<Vec<u8>> {
        ...
        if let Some(lfs_oid) = lfs_pointer_oid(&payload) {
            ...
            return std::fs::read(&path)
                .map_err(|_| OrigoError::NotFound(format!("lfs object {lfs_oid}")));
        }
        Ok(payload)
    }
}
