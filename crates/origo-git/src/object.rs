...
//! **SHA-256** (origo's native 256-bit story). We encode and decode both, and pick
...
pub enum ObjectFormat {
    ...
    Sha1,
    /// 256-bit SHA-256 ids — matches origo's 256-bit content addressing.
    Sha256,
}
...
pub fn validate_oid(oid_hex: &str) -> Result<()> {
    ...
    if !ok {
        return Err(OrigoError::Content(format!(
...
pub fn read_loose(git_dir: &Path, oid_hex: &str) -> Result<(String, Vec<u8>)> {
    ...
    let compressed =
        std::fs::read(&path).map_err(|_| OrigoError::NotFound(format!("git object {oid_hex}")))?;
    let mut framed = Vec::new();
    // Bounded inflate: cap the output so a decompression bomb can't OOM us.
    flate2::read::ZlibDecoder::new(&compressed[..])
        .take(MAX_GIT_OBJECT + 1)
        .read_to_end(&mut framed)
        .map_err(|e| OrigoError::Content(format!("inflate {oid_hex}: {e}")))?;
...
pub fn parse_framed(framed: &[u8]) -> Result<(String, Vec<u8>)> {
    let bad = || OrigoError::Content("malformed git object header".to_string());
...
pub fn parse_tree(payload: &[u8], fmt: ObjectFormat) -> Result<Vec<ParsedTreeEntry>> {
    let bad = || OrigoError::Content("malformed git tree".to_string());
...
pub fn parse_commit(payload: &[u8]) -> Result<ParsedCommit> {
    let bad = || OrigoError::Content("malformed git commit".to_string());
...
/// Render an origo author string as a git ident (`Name <email>`), synthesizing an
/// email when the origo author carries none so `git fsck` stays happy.
pub fn git_ident(author: &str) -> String {
    if author.contains('<') && author.contains('>') {
        author.to_string()
    } else {
        format!("{author} <{author}@origo.local>")
    }
}
