//! Drive the real `git` binary against an origo workspace through the
//! `git-remote-origo` helper: clone from origo, then edit + commit + push back, and
//! confirm the pushed history and content land in origo.
...
const HELPER: &str = env!("CARGO_BIN_EXE_git-remote-origo");
...
async fn clone_edit_push_roundtrip() {
    ...
    // Seed an origo workspace with two files and a commit, then release it so the
    ...
    let url = format!("origo://{}", ws_path.display());
    ...
    // --- edit + commit + push back into origo --------------------------------
