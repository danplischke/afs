//! origo-mcp — expose an origo workspace to agents over the Model Context Protocol
//! (`docs/DESIGN.md` §4e).
//!
...
use origo_sdk::{OrigoError, Workspace, WriteCtx};
use serde_json::{json, Value};

type Result<T> = std::result::Result<T, OrigoError>;
        ...
        match method {
            "initialize" => Some(ok(
                ...
                json!({
                    ...
                    "capabilities": { "tools": {} },
                    "serverInfo": { "name": "origo", "version": env!("CARGO_PKG_VERSION") },
                }),
            )),
...
fn tool_defs() -> Vec<Value> {
    let path_prop = json!({ "path": { "type": "string", "description": "absolute origo path" } });
    vec![
        tool(
            "origo_read",
        ...
        tool("origo_log", "Show commit history.", json!({}), &[]),
    ]
}
