...
async fn initialize_and_list_tools() {
    ...
    let init = s
        ...
        .await
        .unwrap();
    assert_eq!(init["result"]["serverInfo"]["name"], "origo");
    ...
    let names: Vec<&str> = list["result"]["tools"]
        ...
        .collect();
    assert!(names.contains(&"origo_write"));
    assert!(names.contains(&"origo_blame"));
}
...
async fn writes_are_attributed_to_the_agent() {
    ...
    let r = s
        .handle(call("origo_read", json!({"path":"/notes.txt"})))
        .await
        .unwrap();
    ...
    let b = s
        .handle(call("origo_blame", json!({"path":"/notes.txt"})))
        .await
        .unwrap();
...
async fn tool_errors_are_reported_not_thrown() {
    let s = server().await;
    let r = s
        .handle(call("origo_read", json!({"path":"/missing"})))
        .await
        .unwrap();
...
async fn serves_over_a_stream() {
    ...
    let input = concat!(
        ...
        "\n",
        r#"{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"origo_mkdir","arguments":{"path":"/d"}}}"#,
        "\n",
    );
    ...
    assert_eq!(responses[0]["result"]["serverInfo"]["name"], "origo");
