"""A complete, runnable origo sample app: a human and an AI agent co-writing a
...
way a real service would:
  ...
  * **Bring-your-own auth.** A bearer token is resolved to an origo *actor* — the
  ...
  * **Identity mapping with no side table.** `find_or_create_human` /
    `find_or_create_agent` map your external user/agent ids onto origo actors
...
2. As a self-contained demo — no server, no curl, nothing to set up::
   ...
   narrating each step. Great for seeing what origo actually does in ~5 seconds.

By default the workspace is a throwaway temp directory. Point it at a real
store with env vars: ``ORIGO_WORKSPACE=/srv/ws`` (local), or ``ORIGO_DSN=postgres://…``
...
# an agent token here instead. Each principal carries a stable external id; origo
...
_ctx_cache: dict[str, origo.WriteCtx] = {}


async def _ctx_for_token(ws: origo.Workspace, token: str) -> origo.WriteCtx:
    """Map a token to the origo actor its writes are attributed to, creating the
...
async def authn(authorization: Optional[str] = Header(default=None)) -> origo.WriteCtx:
    """The auth dependency handed to ``build_router``: resolve
    ``Authorization: Bearer <token>`` to the origo actor its writes are
...
_WS: origo.Workspace  # set on startup, read by `authn`
...
async def lifespan(app: FastAPI):
    ...
    if dsn:
        ws = await origo.Workspace.open_pg(dsn, os.environ.get("ORIGO_CAS", "cas"))
    else:
        ws_dir = os.environ.get("ORIGO_WORKSPACE")
        if ws_dir is None:
            tmp = tempfile.TemporaryDirectory(prefix="origo-collab-")
            ws_dir = tmp.name
        os.makedirs(ws_dir, exist_ok=True)
        ws = await origo.Workspace.open_local(f"{ws_dir}/meta.db", f"{ws_dir}/cas")
...
app = FastAPI(title="origo collab", lifespan=lifespan)
...
async def feed(request: Request, since: int = 0):
    ...
    ws: origo.Workspace = request.app.state.ws
