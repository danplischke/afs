"""Example: mount the whole origo workspace API with your own auth.

`origo.fastapi.build_router` gives you every workspace endpoint — files, blame,
...
provide. origo has no built-in auth on purpose: a blame trail is only trustworthy
...
# --- your auth --------------------------------------------------------------
# Resolve the request's principal to the origo actor it should be attributed to.
# Swap the header check for your real auth (decode a JWT, look up a session,
# validate an agent token) and map that principal to an origo actor id. The
...
# to the origo actor your `authn` will later resolve to. find_or_create_human is
