//! Python bindings for origo — an async-native module for driving a workspace
...
create_exception!(origo, OrigoError, pyo3::exceptions::PyException);
create_exception!(origo, ConflictError, OrigoError);

/// Map an origo error onto the closest Python exception.
fn to_pyerr(e: origo_sdk::OrigoError) -> PyErr {
...
fn unsupported(what: &str) -> PyErr {
    PyOSError::new_err(format!(
        "{what} is not available on this platform (Unix/FUSE only); use the HTTP API (origo.fastapi) or embed the SDK"
    ))
}
...
fn hash_opt(h: Option<&origo_sdk::Hash>) -> Option<String> {
    h.map(|h| h.to_hex())
}
...
/// An origo workspace. Open one with a classmethod, then drive it with async
...
impl Workspace {
    ...
    /// origo actor without keeping a side table.
    ...
    /// The metadata DB's schema state as `{current, latest, up_to_date}`. origo
    ...
    fn mount(&self, py: Python<'_>, mountpoint: String) -> PyResult<Mount> {
        ...
        let session = py
            .detach(move || origo_fuse::spawn(ws, Path::new(&mp)))
            .map_err(io_err)?;
        Ok(Mount {
    ...
    /// HTTP API (`origo.fastapi`) or embed the SDK directly.
    #[cfg(not(unix))]
    ...
    fn serve_nfs<'py>(&self, py: Python<'py>, addr: String) -> PyResult<Bound<'py, PyAny>> {
        let ws = self.inner.clone();
        future_into_py(py, async move {
            origo_nfs::serve(ws, &addr).await.map_err(io_err)?;
            Ok(())
        })
    }

    /// NFS serving is not available on this platform (Unix only). Use the HTTP
    /// API (`origo.fastapi`) or embed the SDK directly.
    #[cfg(not(unix))]
...
/// The compiled extension is imported as `origo._origo`; the pure-Python package
/// `origo` (see `python/origo/__init__.py`) re-exports everything from it and adds
/// optional integrations like `origo.fastapi`.
#[pymodule]
fn _origo(m: &Bound<'_, PyModule>) -> PyResult<()> {
    ...
    m.add("OrigoError", m.py().get_type::<OrigoError>())?;
