...
Run (from crates/origo-py, after `maturin develop`):
...
def test_rebuild_recovers_files_and_branches():
    async def _run():
        with tempfile.TemporaryDirectory() as d:
            cas = str(Path(d) / "cas")
            ws = await origo.Workspace.open_local(str(Path(d) / "meta.db"), cas)
            ...
            recovered = await origo.Workspace.open_local(str(Path(d) / "meta2.db"), cas)
