//! origo-sandbox — run an unmodified process against an **isolated copy-on-write
//! view** of an origo workspace, then import what it changed back as an attributed
...
//! 4. **Import** that delta back into origo via attributed writes (blame + edit-op),
//!    or `--discard` it.
//!
//! The kernel overlay is the disposable *scratch*; origo's own object graph is the
...
//! `meta.db`/`cas`), no network namespace, and no seccomp. We only strip origo's
//! own `ORIGO_ENCRYPTION_KEY` from the child; everything else your process can
//! reach, the command can too.
//!
//! **Isolated (`isolate: true`, `origo sandbox --isolate`) — a real filesystem
...
use origo_sdk::{FileKind, Workspace, WriteCtx};
...
pub struct LiveOpts {
    ...
    /// Working root for `lower/upper/work/merged` (a temp dir).
    pub work_root: PathBuf,
    /// How often to sync the agent's changes into origo while it runs.
...
/// changes into origo (attributed) **live** — every `sync_interval` while it runs,
...
/// its writes land in `upper/`, which a [`LiveSync`] on the host imports into origo
...
pub async fn run_live(ws: &Workspace, opts: LiveOpts, cmd: &[String]) -> Result<Outcome> {
    ...
    // Sync the agent's changes into origo on the timer until it exits. A missed
...
fn overlay_command(
    ...
    command
        // Don't hand origo's own at-rest encryption key to the child: the process
        ...
        .env_remove("ORIGO_ENCRYPTION_KEY")
        .args(["-U", "-r", "-m", "/bin/sh", "-c", SCRIPT, "origo-sandbox"])
        .arg(lower)
        .arg(upper)
        .arg(work)
        .arg(merged);
...
const BWRAP_WORKDIR: &str = "/origo-work";
...
fn join_origo(dir: &str, name: &str) -> String {
    if dir == "/" {
        format!("/{name}")
    } else {
        format!("{dir}/{name}")
...
/// Recursively write the origo tree rooted at `origo_dir` into the host `host_dir`.
async fn export_tree(ws: &Workspace, origo_dir: &str, host_dir: &Path) -> Result<()> {
    tokio::fs::create_dir_all(host_dir).await?;
    for e in ws.ls(origo_dir).await? {
        // Defense-in-depth: origo now rejects these names at the metadata boundary,
        ...
        let child_origo = join_origo(origo_dir, &e.name);
        let child_host = host_dir.join(&e.name);
        match e.kind {
            FileKind::Dir => {
                Box::pin(export_tree(ws, &child_origo, &child_host)).await?;
            }
            FileKind::File => {
                let bytes = ws.read(&child_origo).await?;
                tokio::fs::write(&child_host, &bytes).await?;
            }
            FileKind::Symlink => {
                let target = ws.readlink(&child_origo).await?;
...
async fn import_delta(
    ...
    while let Some(entry) = rd.next_entry().await? {
        ...
        let origo_path = format!("/{}", rel.to_string_lossy());
        ...
        if ft.is_char_device() && md.rdev() == 0 {
            ...
            let _ = origo_rm_rf(ws, &origo_path).await;
            count += 1;
        } else if ft.is_dir() {
            ws.mkdir_p(&origo_path).await?;
            count += Box::pin(import_delta(ws, root, &host, actor, session)).await?;
        } else if ft.is_symlink() {
            let target = tokio::fs::read_link(&host).await?;
            let _ = ws.remove(&origo_path).await;
            ws.symlink(&target.to_string_lossy(), &origo_path).await?;
            count += 1;
        } else if ft.is_file() {
            let bytes = tokio::fs::read(&host).await?;
            match (actor, session) {
                (Some(a), Some(s)) => {
                    ws.write_as(WriteCtx::session(a, s), &origo_path, &bytes)
                        .await?
                }
                _ => ws.write(&origo_path, &bytes).await?,
            }
            count += 1;
        }
    }
...
/// A stateful, incremental sync of an overlay `upper/` delta into origo.
///
...
/// agent's edits into origo (attributed, on the change feed) as it works, instead
...
pub struct LiveSync {
    /// `origo_path -> (mtime_ns, size)` last imported.
...
impl LiveSync {
    ...
    /// number of origo paths mutated this round (0 when the agent is idle).
    ...
    async fn sync_dir(&mut self, ws: &Workspace, root: &Path, dir: &Path) -> Result<usize> {
        ...
        while let Some(entry) = rd.next_entry().await? {
            ...
            if ft.is_char_device() && md.rdev() == 0 {
                ...
                if self.deleted.insert(origo_path.clone()) {
                    let _ = origo_rm_rf(ws, &origo_path).await;
                    self.seen.remove(&origo_path);
                    count += 1;
                }
            } else if ft.is_dir() {
                ws.mkdir_p(&origo_path).await?;
                ...
                if self.seen.get(&origo_path) != Some(&key) {
                    let target = tokio::fs::read_link(&host).await?;
                    let _ = ws.remove(&origo_path).await;
                    ws.symlink(&target.to_string_lossy(), &origo_path).await?;
                    self.seen.insert(origo_path.clone(), key);
                    self.deleted.remove(&origo_path);
                    count += 1;
                }
            } else if ft.is_file() {
                let key = (mtime_ns(&md), md.len());
...
/// Recursively remove an origo path (file or directory).
async fn origo_rm_rf(ws: &Workspace, path: &str) -> Result<()> {
    match ws.stat(path).await {
        Ok(inode) if inode.kind == FileKind::Dir => {
            for e in ws.ls(path).await? {
                Box::pin(origo_rm_rf(ws, &join_origo(path, &e.name))).await?;
            }
