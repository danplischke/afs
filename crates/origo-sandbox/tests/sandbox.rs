...
//! its delta (create / modify / delete) back into origo with attribution.
//!
//! Self-skips where unprivileged overlayfs isn't available.

use origo_sandbox::{overlay_supported, run, LiveSync, RunOpts};
use origo_sdk::{ActorKind, Workspace};
...
/// `LiveSync` streams an overlay `upper/` delta into origo, importing only what
...
/// origo *during* the run (on the sync timer) and once at exit — creates, edits
/// (copy-up + append), and deletes (whiteout) all land, attributed to the agent.
#[tokio::test]
async fn run_live_streams_changes_to_origo() {
    if !overlay_supported() {
        eprintln!("skipping: unprivileged overlayfs unavailable");
        return;
    }
    ...
    let out = origo_sandbox::run_live(
        &ws,
        origo_sandbox::LiveOpts {
            actor: Some(agent),
    ...
    // origo reflects every change, attributed to the agent.
    assert_eq!(&ws.read("/new.txt").await.unwrap()[..], b"created\n");
    assert_eq!(
        &ws.read("/keep.txt").await.unwrap()[..],
        b"original\nmore\n"
