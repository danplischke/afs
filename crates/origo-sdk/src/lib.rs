//! origo-sdk — an ergonomic front door to an origo workspace.
//!
...
pub use origo_core::{
    ...
    Hash, Inode, MemStore, MergeOutcome, ObjectContentStore, OrigoError, PackStore, Presence,
...
impl Workspace {
    ...
    /// memory-bounded counterpart to [`Self::read`] for large files (origo imposes
    ...
    pub async fn merge_branch(
        ...
        let target = self
            ...
            .find(|(n, _)| n == name)
            .map(|(_, h)| h)
            .ok_or_else(|| OrigoError::NotFound(format!("branch {name}")))?;
    ...
    pub fn latest_schema_version(&self) -> i64 {
        origo_core::latest_schema_version()
    }
    ...
    /// `NOTIFY origo_events` so consumers can be pushed instead of polling).
    ...
    pub async fn subscribe(
        ...
        match &self.pg {
            ...
            None => Err(OrigoError::InvalidArgument(
...
async fn read_or_create_salt(cas_dir: &Path) -> Result<Vec<u8>> {
    ...
    getrandom::getrandom(&mut salt)
        .map_err(|e| OrigoError::Content(format!("failed to generate encryption salt: {e}")))?;
