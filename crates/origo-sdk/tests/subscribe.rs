...
//! Self-skips unless `ORIGO_PG_TEST_URL` points at a reachable database.

use origo_sdk::{MemStore, Workspace};
use std::sync::Arc;
...
async fn subscribe_pushes_writes_over_postgres() {
    let Some(dsn) = dsn() else {
        eprintln!("skipping subscribe_pushes_writes_over_postgres: ORIGO_PG_TEST_URL unset");
        return;
    };
