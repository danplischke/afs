...
| "Versioning" = overlay layers + `cp agent.db` | No named branches, no history DAG, no merge | **Git-style commit DAG**, branches/refs, **three-way merge** with conflict handling |
...
| **M3 — Versioning core** | commit/tree/blob objects; refs + reflog; `commit`/`log`/`checkout`/`diff`; overlay working tree over a base commit | Real history + branches (goal 3, part 1) |
...
| **M5 — Attribution** | actor/session registry; `edit_op` capture wired through the write path; `blame` index; blame API + xattr; merge/rebase blame survival | **Per-actor attribution + blame** (goal 4) |
| **M6 — Access surfaces** | FUSE mount; MCP server (auto-attributed tool calls); HTTP/gRPC control API; NFS for macOS | Agents + humans actually use it as a filesystem |
| **M7 — Live collaboration** | shared working tree with advisory-lock coordination; `watch`/NOTIFY; opt-in CRDT co-editing; offline solo + reconnect-merge | **Shared human+agent live workspace** (goal 4, live) |
| **M8 — Hardening** | GC (mark-sweep + grace); encryption/compression; metrics/tracing; benchmarks; migration tool from an agentfs `.db` | Production readiness |
...
| snapshot = `cp agent.db` | **commit** object in a DAG; branches = `ref` |
