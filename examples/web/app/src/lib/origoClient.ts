// A typed client over the origo document server. Reads hit the origo router (/fs)
...
/** Encode an origo path ("/dir/a.md") into a `{path:path}` URL suffix. */
...
export class OrigoError extends Error {
  constructor(
    message: string,
    readonly status: number,
  ) {
    super(message);
    this.name = "OrigoError";
  }
}

export class OrigoClient {
  ...
  private async error(res: Response): Promise<OrigoError> {
    ...
    return new OrigoError(detail, res.status);
  }
  ...
  async commit(message: string, author = "origo-web"): Promise<string> {
   ...
   * origo router; the inline review reads the proposed content back from origo via
  ...
  /**
   * Keep the chosen hunks. Keep-all → origo's native accept (credits the agent);
